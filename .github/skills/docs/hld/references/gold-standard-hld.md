...
type: design
status: current
feature: invoice-composition-replacement
...
<!--
GOLD STANDARD REFERENCE for the hld skill.
This is a real HLD whose organisation-specific facts (company, vendors, hosts, paths, customer
volumes) were replaced with neutral ones so it can ship publicly. Everything the skill copies
from it is unchanged: the 8-section structure, heading numbering, the diagram types and their
styling, the ID schemes (SYS_nn, INT_nn, REQ-nn), and every table's columns. Match THAT, not
the domain.
-->

# High-Level Architecture (HLA): Invoice Composition Replacement Platform

## 1. Executive Summary

### 1.1 Initiative Goal
The Invoice Composition Replacement Platform replaces the proprietary third-party invoice composition system that a telecommunications operator uses for fixed-line billing.
...
- It processes approximately 100,000 customer accounts per billing run within a four-hour service level agreement (SLA) window.
...
### 2.1 Background
The operator's fixed customer billing produces monthly invoices across four staggered billing cycles. The legacy composition workflow relied on an external vendor system. This introduced high annual licensing costs and operational delays whenever regulatory or template changes occurred. The replacement platform moves invoice composition entirely onto internal infrastructure using high-performance open standards.

### 2.2 Scope

#### 2.2.1 In Scope
- Ingestion of raw `.EN` tagged-record stream files generated by the upstream billing engine.
...
#### 2.2.2 Out of Scope
- Upstream rating, charging, or billing account balance calculation performed by the billing engine.
...
- **Local Filesystem Isolation**: Network-isolated execution on `batch-host-01` with zero runtime external API or HTTP dependencies.
...
sequenceDiagram
    ...
    Orch->>Audit: Verify population counts (PDF == XML == AFP == 100,000)
...
flowchart TD
    ...
    subgraph Upstream["Upstream Billing Systems"]
        direction LR
        SYS_01["SYS_01: Billing & Rating Engine<br/>Rating & Billing Lifecycle"]:::existingStyle
    end

    subgraph CorePlatform["Invoice Composition Replacement Platform (batch-host-01)"]
        direction LR
        SYS_02["SYS_02: Composition Replacement Engine<br/>Ingestion, Mapping, Reconcile & Render"]:::newStyle
...
|---|---|---|---|---|
| `SYS_01` | Billing & Rating Engine | Billing / Rating | Upstream rating engine producing periodic `.EN` billing stream files. | No |
| `SYS_02` | Composition Replacement Engine | Batch Engine | Core batch orchestration, mapping, financial reconciliation, and multi-channel rendering engine. | Yes |
...
| `SYS_04` | Customer Self-Service Portal | Web Portal | Customer billing portal ingesting structured eBilling XML for web presentation. | No |
...
#### 4.2.1 SYS_01: Billing & Rating Engine
The billing engine calculates customer monthly billing totals, rating, and payment adjustments. At cycle close, it exports raw `.EN` tagged-record stream files to the staging transfer area.

#### 4.2.2 SYS_02: Composition Replacement Engine
...
#### 4.2.4 SYS_04: Customer Self-Service Portal
...
|---|---|---|---|---|---|
| `INT_01` | SFTP / SMB File Feed | `SYS_01` | `SYS_02` | Tagged-record billing stream (`.EN`) and cycle marker file. | Existing Connectivity |
...
#### 4.3.1 INT_01: Billing Stream & Completion Marker
...
Transfers byte-deterministic PDF documents containing vector brand artwork, billing transaction tables, and payment slips.
...
The engine executes entirely within a dedicated local filesystem hierarchy on host `batch-host-01`. Every batch run is isolated by unique run and attempt identifiers:
...
- **PDF Documents**: 100,750 total PDF files (100,000 primary invoices + 750 secondary itemized schedules).
- **XML Documents**: 100,000 structured customer eBilling files.
- **AFP Spools**: 32 consolidated MO:DCA print files matching the 32 upstream input batches.
...
- All fonts, styling assets, and configuration templates reside locally on disk on `batch-host-01`.
...
- Upstream customer metadata categories (`FIRSTNAME`, `EMAIL`, `OFFNETID`, `POSTCODE`) are dropped during ingestion mapping.
...
|---|---|---|---|
| `REQ-01` | Ingest `.EN` tagged-record streams | `SYS_02` (`ingestion/en_reader.py`) | Automated parser unit tests and zero-failure batch validation. |
...
| `REQ-04` | Multi-channel parity (PDF/XML/AFP/JRN) | `SYS_02` (`mapping/*`, `rendering/*`) | Cross-output population count verification (`100,000` exact). |
...
| Term | Definition |
|---|---|
| `.EN` | Proprietary tagged-record flat file format emitted by the upstream billing engine. |
...
- `<repo>/README.md`: System operational runbook and architecture overview.
- `<repo>/docs/SECURITY_AND_PRIVACY.md`: Security controls and privacy safeguards.
- `<repo>/docs/PRODUCTION_CANDIDATE_ATT006.md`: Candidate certification report and benchmark results.
