...
# Internal project names must not reach the PUBLIC mirror. saajunaid/caddis is private;
# saajunaid/caddis-plugin is public and receives every exported bundle, including the shipped
# hook tests. On 2026-09-09 six tracked files there named five internal projects — one added the
# previous day, in a commit that never flagged it.
#
# The incidents keep their teaching value without the names: the numbers, the mechanism and the
# failure shape are what make them useful, not which repo they happened in.
#
# `vmie` is deliberately NOT on this list. It is the company, not a project, and the GitHub org
# is public anyway; blind-replacing it would mangle prose. That is a separate decision.
INTERNAL_NAMES = (
    "serve-sight", "rev-sight", "app-sight", "ops-sight", "uni-sight", "intel-sight",
    "watch-sight", "nps-lens", "platform-infra", "Customer360", "ucip-framework",
...
def check_no_internal_names_in_public_source() -> CheckResult:
    ...
    result = CheckResult(name="Internal project names — must not reach the public mirror")
    roots = [
        GITHUB_DIR / "skills",
        GITHUB_DIR / "agent-docs",
        REPO_ROOT / "claude-harness",
    ]
    seen = 0
    for root in roots:
        if not root.exists():
            continue
        for f in root.rglob("*"):
            if not f.is_file() or f.suffix not in (".md", ".py", ".json", ".tmpl"):
                continue
            if ".archive" in str(f):
                continue
            try:
            ...
            for name in INTERNAL_NAMES:
                if name in text:
                    ...
                    result.failures.append(
                        f"{rel} names '{name}' and ships to the PUBLIC mirror — "
                        "describe the incident without the repo name"
                    )
    result.passed = not result.failures
    result.info.append(f"scanned {seen} mirror-bound file(s)")
    return result



def check_second_brain_stays_private() -> CheckResult:
    """The fleet instrument must never reach the public mirror.

    `.caddis/second-brain.html` names 18 internal repos and two internal hostnames. The source
    repo is private; caddis-plugin is PUBLIC. This check exists because the page is a tempting
    thing to publish — it is the best-looking artefact in the repo — and one copy would expose
    the whole fleet's plans and open asks.
    """
    result = CheckResult(name="Second brain — must stay OUT of the public mirror")
    mirror = REPO_ROOT / "vscode-extensions" / "caddis-plugin"
    for base in (REPO_ROOT / "dist" / "runtime-resources", mirror):
        if not base.exists():
            continue
        for f in base.rglob("second-brain*"):
            if ".git" in f.parts:
                continue
            result.failures.append(
                f"{f.relative_to(REPO_ROOT).as_posix()} — the fleet page names internal repos "
                "and must not ship"
            )
    local = REPO_ROOT / ".caddis" / "second-brain.html"
    if local.exists():
        import datetime
        age = (datetime.datetime.now()
               - datetime.datetime.fromtimestamp(local.stat().st_mtime)).days
        result.info.append(f"local page is {age} day(s) old (regenerated by caddis-push)")
        if age > 30:
            result.info.append("  stale — run `python scripts/second_brain.py`")
    else:
        result.info.append("no local page yet — caddis-push will generate one")
    result.passed = not result.failures
    return result
...
def check_command_stage() -> CheckResult:
    """Every command declares the stage it belongs to.

    The reference site groups by this. It used to be regexed out of docs/guide/caddis.md, and
    four commands were invisible there because the guide had never listed them — it still says
    "All 28" against 31. Moving the fact into the command means a new one cannot land ungrouped:
    this check fails before it can.
    """
    result = CheckResult(name="Command stage — every command declares where it belongs")
    cmds = REPO_ROOT / "claude-harness" / "commands"
    n = daily = 0
    for f in sorted(cmds.glob("*.md")):
        text = f.read_text(encoding="utf-8")
        if not text.startswith("---"):
            result.failures.append(f"{f.name}: no frontmatter")
            continue
        head = text[3:text.index(chr(10) + "---", 3)]
        meta = {}
        ...
        if not stage:
            result.failures.append(f"{f.name}: no `stage:` — the reference site cannot place it")
        elif stage not in STAGES:
            result.failures.append(
                f"{f.name}: stage '{stage}' is not one of {', '.join(STAGES)}")
        if meta.get("daily", "").lower() == "true":
            daily += 1
    result.info.append(f"{n} command(s), {daily} marked daily")
    result.passed = not result.failures
    return result
...
def check_reference_site() -> CheckResult:
    ...
    caddis-push builds it into the mirror BEFORE this gate runs, so a generator that broke or
    silently produced an old page aborts the publish instead of shipping documentation that
    disagrees with the plugin. That is the failure `second_brain.py` already demonstrated: a
    generator nothing checked went 34 days stale without a single complaint.
