...
> **Shipped as `CADDIS-GLOSSARY.md`.** This defines CADDIS's own vocabulary — the words the
> toolchain coined, which a user meets constantly and cannot guess from the plain word. Your
> project's own terms belong in `.caddis/kb/GLOSSARY.md` instead; run `/caddis:glossary` to
> build one. Two files, two subjects, deliberately different names.
