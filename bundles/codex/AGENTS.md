...
```
1. PLAN      write a phased plan to .claudster/plans/<feature>.md
2. per phase:
...
3. END       update .claudster/relay.md so the next session resumes (read it first on resume)
```
...
`.claudster/agent-docs/` before continuing. This is the primary defense against session overflow.

### Resuming a session
On a fresh session read in order: `.claudster/relay.md` (if present) → the active plan in
`.claudster/plans/` → its tracker. The conversation is disposable; these files are the truth.
...
| Need | Location |
|---|---|
| Reference-doc index (the meta-KB) — **read first** | `.claudster/kb/DOC-MAP.md` |
| Every UI page → endpoints → DB (frontend repos) | `UI_PAGE_GUIDE.md` |
| Active plans | `.claudster/plans/<feature>.md` |
| PRDs | `.claudster/prd/<feature-slug>.md` |
| Standing prompts / drivers / specs handed to an agent | `.claudster/prompts/` |
| Review output (code-review, cross-review) | `.claudster/reviews/` |
| ADRs, design docs, runbooks | `docs/adr/`, `docs/design/`, `docs/runbooks/` (team-facing, not tool scratch) |
| Agent-produced docs (evals, reviews, debug) | `.claudster/agent-docs/` |
| Session resume doc | `.claudster/relay.md` |
| Per-area conventions | `src/AGENTS.md`, `frontend/AGENTS.md`, `tests/AGENTS.md` (as present) |

**`.claudster/` is the default home for working artifacts.** A plan → `.claudster/plans/`, a PRD →
`.claudster/prd/`, the KB → `.claudster/kb/`, a prompt/driver/spec → `.claudster/prompts/`, a review →
`.claudster/reviews/`. Interpret an unqualified reference ("write a plan", "save this prompt")
...
Read `.claudster/kb/DOC-MAP.md` first to find the right reference doc, then read it on demand. When you
