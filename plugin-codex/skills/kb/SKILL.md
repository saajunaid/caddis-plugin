...
```
python "${CADDIS_PLUGIN_ROOT}/scripts/check_doc_coverage.py" --reindex
```
...
- If it reported **dangling** links (a linked file that's gone), decide per link:
  ...
  - The note is **gone for good** → remove its row. To clear all dangling rows at once, use the
    ...
    ```
    python "${CADDIS_PLUGIN_ROOT}/scripts/check_doc_coverage.py" --prune
    ```
...
## Step 4 — verify clean
```
python "${CADDIS_PLUGIN_ROOT}/scripts/check_doc_coverage.py" --check
```
