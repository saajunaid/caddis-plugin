...
```bash
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_gate.py" hub-artifacts --reports .caddis/advisory-hub-reports
```
...
3. **A context self-check the new Hub must answer BEFORE touching a phase**, from the files, including:
   ...
   > python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_spawn.py" handshake record --id <id> --event answers
   > python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_spawn.py" handshake close  --id <id>
   > ```
   >
...
```
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_gate.py" handover-check \
  --doc .caddis/advisory-hub-reports/hub-NN.spawn.md
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_spawn.py" check \
  --doc .caddis/advisory-hub-reports/hub-NN.spawn.md
```
