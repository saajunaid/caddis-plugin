...
- `${CADDIS_PLUGIN_ROOT}/scripts/caddis_migrate_dir.py` — a plugin install.
...
## Step 3 — dry run
```
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_migrate_dir.py" <target>
```
...
## Step 4 — apply
```
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_migrate_dir.py" <target> --apply
```
...
python "${CADDIS_PLUGIN_ROOT}/scripts/caddis_migrate_dir.py" <target> --check    # exit 0 = no legacy dir left
```
